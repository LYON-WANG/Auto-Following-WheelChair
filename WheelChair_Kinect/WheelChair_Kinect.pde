...
int userID;
int hand_num;
int Mode_Flag = 0; //Mode Flag   1:Following Mode  2:Helmet Mode   3:Manual Mode   4:Leap Motion Mode
...
int SpeedL = 0;
int SpeedR = 0;
int SpeedButton = 20;  //Manual Mode Speed
int SpeedLow = 30;
int SpeedMid = 55;
int SpeedHigh = 80;
int Gyroscope = 0;
...
void setup()
{
  ...
  leap = new LeapMotion(this);                     // Setup the Leap Motion
  ...
  //-- Music --//
  Media_Menu     = minim.loadFile("Media_Menu.mp3");      
  ...
  if(Mode_Flag == 1)
  {
    ...
    if (userList.size()>0)
    {
      ...
      if (kinect.isTrackingSkeleton(userID))
      {
        ...
        offset = int(map(torsoPosition.x, -700, 700, -13,13));    //Offset 
        SpeedFollowing = int(map(torsoPosition.z, 1700, 3000, 15, 90));   //Speed with Distance
 ...
 for (Hand hand : leap.getHands ())
  {
    ...
    if(hand_num == 1)
    {
      ...
      if(hand_position.z > 70)
      	hand_position.z = 70;
      Leap_Direction = int(map(hand_position.x, 150,1200,-30, 30));  //Offset
      Leap_Speed = int(map(hand_position.z, 30, 80, -75, 75));    //Forward and Backward
      if((Leap_Direction >= -8) && (Leap_Direction <= 8))
      	Leap_Direction = 0;
      if((Leap_Speed >= -20) && (Leap_Speed <= 20))
      	Leap_Speed = 0;

			if(hand_grab >= 0.5)           //Grab to Start Control
    ...
    if((SpeedR < 0) || (SpeedL < 0))
    {
    	SpeedL = -Leap_Speed;
    	SpeedR = -Leap_Speed;
    	SpeedL = int(map(-Leap_Speed, 0, 60, 0, 40));
    	SpeedR = int(map(-Leap_Speed, 0, 60, 0, 40));
    	Direction = 2;
    }
